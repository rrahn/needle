...
template <class IBFType>
void estimate(arguments const & args, IBFType & ibf, std::vector<float> & expressions, std::filesystem::path file_out,
...
void call_estimate(arguments const & args, std::vector<float> & expressions, std::filesystem::path file_out,
              std::filesystem::path search_file, std::filesystem::path path_in)
{
    if (args.compressed)
    {
        seqan3::interleaved_bloom_filter<seqan3::data_layout::compressed> ibf;
        estimate(args, ibf, expressions, file_out, search_file, path_in);
    }
    else
    {
